...
def user_registration() -> None:
    ...
    if submitted:
        ...
        st.success("사용자 등록이 완료되었습니다. 이제 목표와 학습 수행을 입력할 수 있어요.")
        st.info("왼쪽 메뉴의 사용자 선택에도 새 계정이 추가되었습니다.")


def data_management() -> None:
    hero("데이터 관리", "SQLite에 저장된 데이터를 확인하고 CSV로 내려받을 수 있습니다.")
    st.info(
        "사용자, 목표, 학습 수행 데이터는 로컬 SQLite 파일(growlog.db)에 저장되며 "
        "앱을 다시 실행해도 유지됩니다."
    )
    ...
    if st.button("데모 데이터 초기화"):
        employees, goals, activities = seed_data()
        reset_database(employees, goals, activities)
        refresh_data()
        st.session_state.current_employee_id = st.session_state.employees.iloc[0][
            "employee_id"
        ]
...
def sidebar() -> tuple[str, str]:
    ...
    with st.sidebar:
        ...
        st.divider()
        st.caption("Standalone MVP · SQLite storage")
    return role, page
