...
def seed_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    today = date.today()
    q_start, q_end, _ = quarter_range(today)

    names = [
        ("E001", "김민준", "제품개발본부", "플랫폼팀", "백엔드", "매니저"),
        ("E002", "이서연", "제품개발본부", "플랫폼팀", "프론트엔드", "프로"),
        ("E003", "박지훈", "제품개발본부", "플랫폼팀", "백엔드", "프로"),
        ("E004", "최유진", "제품개발본부", "플랫폼팀", "데이터", "프로"),
        ("E005", "정도윤", "제품개발본부", "플랫폼팀", "DevOps", "프로"),
        ("E006", "한지민", "제품개발본부", "플랫폼팀", "QA", "프로"),
        ("E007", "윤서준", "비즈니스본부", "사업기획팀", "기획", "매니저"),
        ("E008", "강하은", "비즈니스본부", "사업기획팀", "기획", "프로"),
        ("E009", "조현우", "비즈니스본부", "사업기획팀", "영업", "프로"),
        ("E010", "임수아", "비즈니스본부", "사업기획팀", "마케팅", "프로"),
        ("E011", "송예준", "비즈니스본부", "사업기획팀", "재무", "프로"),
        ("E012", "오채원", "비즈니스본부", "사업기획팀", "HR", "프로"),
    ]
    employees = pd.DataFrame(
        names, columns=["employee_id", "name", "division", "team", "job", "level"]
    )

    goal_templates = [
        ("클라우드 아키텍처 역량 강화", "기술", 36),
        ("데이터 기반 의사결정", "기술", 24),
        ("비즈니스 영어 루틴 만들기", "외국어", 30),
        ("리더십 기본기 다지기", "리더십", 20),
        ("직무 전문서 3권 읽기", "독서", 18),
        ("직무 자격증 취득 준비", "자격증", 42),
    ]
    ...
    for idx, employee in employees.iterrows():
        template = goal_templates[idx % len(goal_templates)]
        goal_id = f"G{idx + 1:03d}"
        status = "completed" if idx in (3, 8) else "in_progress"
        goals.append(
            {
                "goal_id": goal_id,
                ...
                "title": template[0],
                "category": template[1],
                "purpose": "현재 직무의 전문성을 높이고 실제 업무에 적용합니다.",
                "start_date": q_start,
                "end_date": q_end,
                "planned_hours": float(template[2]),
                "status": status,
                "created_at": datetime.now(),
            }
        )

        activity_count = max(1, (idx * 3 + 5) % 9)
        if idx == 10:
            activity_count = 0
        for act_idx in range(activity_count):
            days_ago = (act_idx * 7 + idx * 2) % 60
            activity_date = max(q_start, today - timedelta(days=days_ago))
            activities.append(
                {
                    "activity_id": f"A{idx + 1:02d}{act_idx + 1:02d}",
                    "goal_id": goal_id,
                    "employee_id": employee["employee_id"],
                    "activity_date": activity_date,
                    "activity_type": ["강의", "독서", "프로젝트", "스터디"][act_idx % 4],
                    "hours": float([1.0, 1.5, 2.0, 2.5][(idx + act_idx) % 4]),
                    "memo": "핵심 내용을 정리하고 업무 적용 포인트를 기록했습니다.",
                    "created_at": datetime.now(),
                }
            )

    return employees, pd.DataFrame(goals), pd.DataFrame(activities)


def initialize_state() -> None:
    if "employees" not in st.session_state:
        employees, goals, activities = seed_data()
        st.session_state.employees = employees
        st.session_state.goals = goals
        st.session_state.activities = activities
    if "current_employee_id" not in st.session_state:
        st.session_state.current_employee_id = "E002"
    ...
    if submitted:
        if not title.strip():
            ...
            st.session_state.goals = pd.concat(
                [st.session_state.goals, new_goal], ignore_index=True
            )
...
def activity_form(employee_id: str) -> None:
    ...
    if submitted:
        ...
        st.session_state.activities = pd.concat(
            [st.session_state.activities, new_activity], ignore_index=True
        )
...
def data_management() -> None:
    hero("데이터 관리", "현재 세션의 데이터를 내려받거나 데모 데이터로 초기화할 수 있습니다.")
    st.info(
        "이 MVP는 데이터를 브라우저 세션에 보관합니다. 서버 재시작 후에도 데이터를 유지하려면 "
        "다음 단계에서 SQLite 또는 사내 데이터베이스를 연결해야 합니다."
    )
    tabs = st.tabs(["목표", "수행 내역"])
    with tabs[0]:
...
def sidebar() -> tuple[str, str]:
    ...
    with st.sidebar:
        ...
        if role == "임직원":
            pages = ["나의 대시보드", "목표 등록", "수행 기록", "데이터 관리"]
        elif role == "팀 리더":
            pages = ["팀 대시보드", "데이터 관리"]
        else:
            pages = ["전사 대시보드", "데이터 관리"]
        page = st.radio("메뉴", pages)

        st.divider()
        employee_names = dict(zip(employees["name"], employees["employee_id"]))
        default_name = employees.loc[
            employees["employee_id"] == st.session_state.current_employee_id, "name"
        ].iloc[0]
        selected_name = st.selectbox(
            "데모 사용자",
            list(employee_names.keys()),
            index=list(employee_names.keys()).index(default_name),
            help="실서비스에서는 사내 SSO 계정으로 자동 결정됩니다.",
        )
        st.session_state.current_employee_id = employee_names[selected_name]
        ...
        st.divider()
        st.caption("Demo MVP · Session storage")
    return role, page
